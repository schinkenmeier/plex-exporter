...
**Detaillierte Struktur-Dokumentation:** [docs/development/structure.md](docs/development/structure.md)
...
| `config/frontend/frontend.json` | Bevorzugte Laufzeitkonfiguration; wird beim Frontend-Build nach `apps/frontend/public/config/frontend.json` kopiert. |
| `apps/frontend/config/frontend.json` | Optionaler Fallback-Pfad fuer lokale Konfiguration, falls `config/frontend/frontend.json` nicht existiert. |
| `config/frontend/frontend.json.sample` | Bevorzugte Beispielkonfiguration (Fallback: `apps/frontend/config/frontend.json.sample`). |
...
## Lokales Setup: Node-Version

- CI laeuft auf Node `20` (`.github/workflows/ci.yml`).
- Lokal ebenfalls Node `20.x` verwenden (`.nvmrc` im Repo-Root, `engines` in `package.json`).
- Nach Node-Wechsel native Module sauber neu bauen:
  ```bash
  npm ci
  npm rebuild better-sqlite3 --workspace @plex-exporter/backend
  ```

## Konfiguration (`config/frontend/frontend.json`)
...
- Ein Feature-Flag steuert den gesamten Pipeline-Pfad: `apps/frontend/src/features/hero/pipeline.js` liest zuerst einen lokalen Override (`localStorage.feature.heroPipeline`), faellt dann auf `config/frontend/frontend.json` (bzw. `apps/frontend/config/frontend.json` als Fallback) (`heroPipelineEnabled` oder `features.heroPipeline`) zurueck und aktiviert die Pipeline standardmaessig, wenn kein Flag gesetzt ist. Wird die Pipeline deaktiviert, blendet das Frontend automatisch das statische Fallback-Hero ein.
