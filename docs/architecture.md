...
## Hero-Policy & Pipeline (`apps/frontend/src/js/hero/...`)
...
## Filter- und Rendering-Layer (`apps/frontend/src/js/filter.js`, `apps/frontend/src/js/grid.js`)
...
* Watchlist (`apps/frontend/src/js/watchlist.js`)
...
* TMDB-Service (`apps/frontend/src/js/services/tmdb.js`)
...
## Detail-System (`apps/frontend/src/js/modalV3/index.js`)

* **Zentrale Steuerung:**
  * Module importieren `openMovieDetailV3()` und `openSeriesDetailV3()` direkt aus `apps/frontend/src/js/modalV3/index.js`. Beide Funktionen akzeptieren IDs oder bereits geladene Datensätze, kümmern sich um Demo- und TMDB-Hydration und steuern Render-Sessions über Tokens (`startRender()`/`isCurrentRender()`).
...
* **Cinematic Shell (`apps/frontend/src/js/modalV3`)**
...
* **Hilfsmodule & Zusammenspiel:**
  * Staffel-/Episodenlisten rendert `apps/frontend/src/js/modalV3/seasons.js` (Lazy-Poster, Episoden-Badges, Toggle-Verhalten). Das Modal bindet die Ausgabe im Tab „Staffeln“ über `renderSeasons()` ein.
...
* **Rendering-Erweiterungen:** Spezialkarten oder zusätzliche Aktionen können in `apps/frontend/src/js/grid.js` über neue Helper (`cardEl`-Varianten) eingebunden werden. Exportierte Utilities (`renderGrid`, `groupCollectionsIfEnabled`) erleichtern Wiederverwendung.
...
* `apps/frontend/src/js/watchlist.js`
